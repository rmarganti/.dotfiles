...
export default function streamOutputExtension(pi: ExtensionAPI): void {
    pi.registerFlag('stream', {
        description:
            'Stream output to stderr. Comma-separated values: message, thinking, tools, all',
        type: 'string',
        default: 'off',
    });
    ...
    function getStreamOptions(): Set<StreamOption> {
        ...
        if (!flag || flag === 'off' || typeof flag !== 'string')
            return new Set();
        ...
        return new Set<StreamOption>(
            parts.filter((p): p is StreamOption =>
                valid.includes(p as StreamOption)
            )
        );
    }
    ...
    pi.on('tool_execution_start', async (event) => {
        ...
        process.stderr.write(
            `${colors.yellow}[tool] [calling] ${event.toolName}${colors.reset}\n` +
                `${colors.yellow}  ${indentContent(argsDisplay)}${colors.reset}\n`
        );
    });
    ...
    pi.on('tool_execution_update', async (event) => {
        ...
        process.stderr.write(
            `${colors.yellow}[tool] [running] ${event.toolName} → ${sizeStr}${colors.reset}\n`
        );
        hasRunningLine = true;
    });
    ...
    pi.on('tool_execution_end', async (event) => {
        ...
        const indented = indentContent(resultDisplay);

        if (event.isError) {
            process.stderr.write(
                `${colors.red}[tool] [error] ${event.toolName}${colors.reset}\n` +
                    `${colors.red}  ${indentContent(resultDisplay)}${colors.reset}\n\n`
            );
        } else {
            process.stderr.write(
