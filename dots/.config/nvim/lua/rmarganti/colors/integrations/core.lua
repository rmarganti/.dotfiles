...
return {
    ...
    Cursor = { fg = a.fg, bg = p.bg }, -- character under the cursor
    lCursor = { fg = a.fg, bg = p.bg }, -- the character under the cursor when |language-mapping| is used (see 'guicursor')
    CursorIM = { fg = a.fg, bg = p.bg }, -- like Cursor, but used when in IME mode |CursorIM|
    CursorColumn = { bg = p.bg_darker }, -- Screen-column at the cursor, when 'cursorcolumn' is secp.
    CursorLine = { bg = p.bg_darker }, -- Screen-line at the cursor, when 'cursorline' is secp.  Low-priority if foreground (ctermfg OR guifg) is not secp.
    ...
    Folded = { fg = a.fg, bg = p.bg }, -- line used for closed folds
    FoldColumn = { bg = p.none, fg = a.minus2 }, -- 'foldcolumn'
    SignColumn = { bg = p.none, fg = a.minus2 }, -- column where |signs| are displayed
    signcolumnsb = { bg = p.bg, fg = a.fg }, -- column where |signs| are displayed
    substitute = { bg = p.bg, fg = a.fg }, -- |:substitute| replacement text highlighting
    LineNr = { fg = a.minus2 }, -- Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is secp.
    CursorLineNr = { fg = a.plus2, bg = p.bg_darker }, -- Like LineNr when 'cursorline' or 'relativenumber' is set for the cursor line. highlights the number in numberline.
    CursorLineSign = { bg = p.bg_darker }, -- Like LineNr when 'cursorline' or 'relativenumber' is set for the cursor line. highlights the number in numberline.
    CursorLineFold = { bg = p.bg_darker }, -- Like LineNr when 'cursorline' or 'relativenumber' is set for the cursor line. highlights the number in numberline.
    ...
    PmenuThumb = { bg = p.bg }, -- Popup menu: Thumb of the scrollbar.
    Question = { fg = a.fg }, -- |hit-enter| prompt and yes/no questions
    QuickFixLine = { bg = p.bg }, -- Current |quickfix| item in the quickfix window. Combined with |hl-CursorLine| when the cursor is there.
    Search = { fg = p.bg_darker, bg = p.green_dark }, -- Last search pattern highlighting (see 'hlsearch').  Also used for similar items that need to stand oucp.
    IncSearch = { fg = p.bg_darker, bg = p.green_dark }, -- 'incsearch' highlighting; also used for the text replaced with ":s///c"
    CurSearch = { fg = p.bg_darker, bg = p.green_light }, -- The 'incsearch' item currently under the cursor
    ...
    StatusLine = { fg = a.fg, bg = p.bg }, -- status line of current window
    -- StatusLineNC = { fg = a.fg, bg = p.bg }, -- status lines of not-current windows Note: if this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
    TabLine = { bg = p.bg, fg = a.fg }, -- tab pages line, not active tab page label
    TabLineFill = { bg = p.none }, -- tab pages line, where there are no labels
    TabLineSel = { fg = a.fg, bg = p.bg }, -- tab pages line, active tab page label
    Title = { fg = p.plus2 }, -- titles for output from ":set all", ":autocmd" etcp.
    Visual = { bg = p.bg_light }, -- Visual mode selection
    VisualNOS = { bg = p.bg_light }, -- Visual mode selection when vim is "Not Owning the Selection".
    ...
    -- debugging
    debugPC = { bg = p.bg }, -- used for highlighting the current line in terminal-debug
    debugBreakpoint = { bg = p.bg, fg = a.fg }, -- used for breakpoint colors in terminal-debug

    -- illuminate
    illuminatedWord = { bg = p.bg },
    illuminatedCurWord = { bg = p.bg },
