...
# Config: Expects other-files.json in the same directory as this script.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/other-files.json"
...
for mapping in "${mappings[@]}"; do
  ...
  if perl -e "exit !('$REL_PATH' =~ /$pattern/)" ; then
...
for m in "${matches[@]:-}"; do
  case "$deduped" in
    *"|$m|"*) ;; # already in list
    *) matches_deduped+=("$m"); deduped="$deduped|$m|";;
  esac
done
