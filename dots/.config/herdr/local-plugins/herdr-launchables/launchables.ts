...
const contextJson = parseJson<Record<string, unknown>>(process.env.HERDR_PLUGIN_CONTEXT_JSON || '', {});
...
function appendLog(message: string): void {
  fs.appendFileSync(pluginLogFile(), `[${new Date().toISOString()}] ${message}\n`);
}

function herdr(args: string[], options: Parameters<typeof spawnSync>[2] = {}): string {
...
function focusedPaneInfo(paneId?: string): { paneId: string; workspaceId: string; cwd: string } {
  const args = paneId ? ['pane', 'current', '--pane', paneId] : ['pane', 'current'];
  const data = herdrJson<{ result?: { pane?: { pane_id?: string; workspace_id?: string; cwd?: string; foreground_cwd?: string } } }>(args);
...
function getPluginContext(): PluginContext {
  let paneId = process.env.LAUNCHABLES_SOURCE_PANE_ID || contextValue('HERDR_PANE_ID', 'pane_id', 'focused_pane_id');
  let workspaceId = process.env.LAUNCHABLES_WORKSPACE_ID || contextValue('HERDR_WORKSPACE_ID', 'workspace_id');
  let cwd = process.env.LAUNCHABLES_CWD || contextValue('focused_pane_cwd', 'pane_cwd', 'workspace_cwd');
...
function validateLaunchable(name: string, value: unknown): ValidationResult {
  ...
  if (cwd !== undefined && !normalizedCwd) errors.push(`${name}: cwd must be a non-empty string when provided`);
  ...
  if (type === 'background') {
    ...
    return errors.length === 0 && isNonEmptyString(command)
      ? { launchable: { type: 'background', command, ...(normalizedCwd ? { cwd: normalizedCwd } : {}) }, errors }
  ...
  if (type === 'split') {
    ...
    if (!isNonEmptyString(command)) errors.push(`${name}: split.command must be a non-empty string`);
    if (candidate.commands !== undefined) errors.push(`${name}: split must not define commands`);
  ...
  if (type === 'tab') {
    ...
    const validCommands = Array.isArray(commands) && commands.length > 0 && commands.every((item) => isNonEmptyString(item))
      ? commands
      : null;
    if (!validCommands) errors.push(`${name}: tab.commands must be a non-empty array of non-empty strings`);
    if (candidate.command !== undefined) errors.push(`${name}: tab must not define command`);
    if (candidate.direction !== undefined) errors.push(`${name}: tab must not define direction`);
    return errors.length === 0 && validCommands
      ? { launchable: { type: 'tab', commands: validCommands, ...(normalizedCwd ? { cwd: normalizedCwd } : {}) }, errors }
      : { launchable: null, errors };
  }
...
function loadLaunchablesFile(filePath: string, source: LaunchableSource): ResolvedLaunchable[] {
  ...
  for (const [name, value] of Object.entries(parsed as Record<string, unknown>)) {
    ...
    if (!result.launchable) {
      for (const error of result.errors) appendLog(`invalid launchable in ${filePath}: ${error}`);
      continue;
    }
...
export function resolveLaunchableCwd(item: ResolvedLaunchable, currentCwd: string): string {
...
function selectLaunchable(items: ResolvedLaunchable[]): ResolvedLaunchable | null {
  ...
  items.forEach((item, index) => process.stderr.write(`  ${index + 1}) ${item.name} [${item.source}] [${item.launchable.type}]\n`));
  process.stderr.write('Choose launchable: ');
  const choice = fs.readFileSync(0, 'utf8').trim();
  const index = Number.parseInt(choice, 10) - 1;
  return Number.isInteger(index) && index >= 0 && index < items.length ? items[index] || null : null;
}
...
function writeSelection(payload: SelectionPayload): string {
  const filePath = path.join(selectionDir(), `${Date.now()}-${process.pid}-${Math.random().toString(36).slice(2)}.json`);
  fs.writeFileSync(filePath, JSON.stringify(payload, null, 2));
  return filePath;
}

function backgroundLogFile(name: string): string {
  const safeName = name.replace(/[^a-z0-9._-]+/gi, '-').replace(/^-+|-+$/g, '') || 'launchable';
...
function detachApply(selectionPath: string, pickerPaneId: string): void {
  ...
  const args = [path.join(PLUGIN_ROOT, 'launchables.ts'), 'apply', selectionPath];
...
async function waitForPaneGone(paneId: string, timeoutMs = OVERLAY_CLOSE_TIMEOUT_MS): Promise<void> {
  if (!paneId) return;
...
function executeTab(payload: SelectionPayload, cwd: string): void {
  ...
  const commands = resolved.launchable.type === 'tab' ? resolved.launchable.commands : [];
  if (!workspaceId || commands.length === 0) return;

  const tab = herdrJson<{ result?: { tab?: { tab_id?: string }; root_pane?: { pane_id?: string } } }>([
    'tab', 'create', '--workspace', workspaceId, '--cwd', cwd, '--label', resolved.name, '--focus',
  ]);
  const tabId = tab.result?.tab?.tab_id || '';
  let currentPaneId = tab.result?.root_pane?.pane_id || '';
  if (!tabId || !currentPaneId) throw new Error(`failed to create tab for ${resolved.name}`);
  ...
  commands.forEach((command, index) => {
    ...
    const split = herdrJson<{ result?: { pane?: { pane_id?: string } } }>([
      'pane', 'split', currentPaneId, '--direction', 'right', '--cwd', cwd, '--no-focus',
    ]);
...
function executeSplit(payload: SelectionPayload, cwd: string): void {
  ...
  const split = herdrJson<{ result?: { pane?: { pane_id?: string } } }>([
    'pane', 'split', sourcePaneId, '--direction', direction, '--cwd', cwd, '--focus',
  ]);
...
function executeSelection(payload: SelectionPayload): void {
  ...
  switch (payload.resolved.launchable.type) {
    case 'background':
      executeBackground(payload.resolved.name, payload.resolved.launchable.command, cwd);
      return;
    case 'tab':
...
async function cmdOpen(): Promise<void> {
  ...
  if (pluginContext.workspaceId) args.push('--env', `LAUNCHABLES_WORKSPACE_ID=${pluginContext.workspaceId}`);
  if (pluginContext.paneId) args.push('--env', `LAUNCHABLES_SOURCE_PANE_ID=${pluginContext.paneId}`);
  if (pluginContext.cwd) args.push('--env', `LAUNCHABLES_CWD=${pluginContext.cwd}`);
...
async function cmdApply(selectionPath: string, pickerPaneId: string): Promise<void> {
  if (!selectionPath) {
    process.stderr.write('usage: launchables.ts apply <selection-path> [picker-pane-id]\n');
    process.exitCode = 2;
    return;
  }
